...
speckleEnvelope = load([speckleDir,'envData.mat']);
...
snrEnv = mEnv./sEnv;
speckleMean = mean(speckleEnvelope.envAll,2);
speckleStd  = std(speckleEnvelope.envAll,0,2);
speckleSNR =  speckleMean./speckleStd;
...
xlim([1 128])
ylim([0 3])
errorbar(120,mean(speckleSNR),std(speckleSNR),'o','MarkerFaceColor','red')

subplot(2,2,4)
hEnvSpeckle = histogram(speckleSNR,'Normalization','probability');
