...
for iImage = 1:nImages
    ...
    nKkernels = kLength_BSC_samples/10;
    %% BSC + Coherence Calc
    ...
    for iLine2 = 1:length(kIdxs{iImage})
        ...
        kernelVal = zeros(12,33);
    
        for iKernel = 1:nKkernels 
            kernelIdxs = axIdxs((iKernel-1)*10 + (1:10));
            kernelVal(iKernel,:) = CoherenceAnalysisFN(squeeze(channelStack(kIdxs{iImage}(iLine2),kernelIdxs,:)));
        end
        
       cohAll(iLine2 + rayCount ,:) = mean(kernelVal,1);
...
saveDir2 = [saveDir,'/COSIEoutput_adaptive_smallK'];
