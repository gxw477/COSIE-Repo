...
for iFrame = 5
    ...
    load([topDir,'ImgData',num2str(iFrame),'.mat'])
    ...
    for i = 1:P.numRays
        ...
        %[bfSig2, channelData2,~] = mustUGeorge2(acqI, zeros(1,length(bformY)), bformY.*lambda,tDelay(~omitBool),params,'linear');
    

        %bfSig2 = das(acqI,reconX(i).*lambda.*ones(1,length(reconY)),reconY.*lambda,tDelay,params,'nearest');
    ...
    iPix = 1;
        ...
        for iK = 1:nKernels
            ...
            RMat(iRay,iK,:) = CoherenceAnalysisFN_new(scatData);
            iPix = iPix + kLength;
    ...
    clearvars -except iFrame topDir
