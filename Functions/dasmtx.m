...
%       M = dasmtx([size(sig3D,1) size(sig3D,2)],x,z,delays,param);
%       % (or M = dasmtx(1i*[size(sig3D,1) size(sig3D,2)],...) for I/Q data)
...
%
%   -- Damien Garcia -- 2017/10, last update 2022/12/14
%   website: <a
...
%       of the transducer elements. It must be a row vector or a matrix
%       with 2 rows corresponding to the x- (and optionally z-)
...
if isfield(param,'elements')
    assert(ismatrix(param.elements) &&...
        any(ismember([1 2],size(param.elements,1))),...
        ['PARAM.elements must be a row vector or 2-row matrix ',...
...
clear x z dTX dRX dxT cosRX tanRX
