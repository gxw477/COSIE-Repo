...
for iImage = 1:7
    ...
    figure
    imagesc((xVals-mean(xVals)).*1e2,yVals.*1e2,bm)
    xlabel('Lateral (cm)')
    ylabel('Axial (cm)')
    axis equal 
    axis tight
    ...
    for iEMLidx = 1:nEMLidx
        ...
        for iDepth = 1:nDepth
            ...
            %segBoolCOH = allCOH(:,iDepth) > speckleCOSIE.EML(1,(iEMLidx-1)*idxFactor+1) &  allCOH(:,iDepth) < speckleCOSIE.EML(2,(iEMLidx-1)*idxFactor+1);    
            ...
            %segBoolSNR = allSNR(:,iDepth) > speckleCOSIE_SNR.EML(1,(iEMLidx-1)*idxFactor+1) & allSNR(:,iDepth) < speckleCOSIE_SNR.EML(2,(iEMLidx-1)*idxFactor+1);    
            ...
            if allDepths(iDepth) > 1e3*( liverStart + wSizeCM*0.01) 
                ...
                attLiverFiltSNR= attLiverStruct.a0 + attLiverStruct.alpha_filt*vsxParams.Trans.frequency;% + attLiverStruct.a0;
