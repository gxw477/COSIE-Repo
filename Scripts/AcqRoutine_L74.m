...
P.startDepth = 5;
P.endDepth = 192;
P.txFocus = Trans.elevationFocusMm*1e-3/lambda;  % Initial transmit focus.
...
n128 =  128*ceil((P.endDepth- P.startDepth)*2*sPerWaveInit/128);
...
Resource.InterBuffer(1).numFrames = 5;  % one intermediate buffer needed.
...
% - We need 128 Receives for every 
...
% - RF + IQ save
UI(5).Control = {'UserA2','Style','VsPushButton','Label','save IQ and RF'};
UI(5).Callback = text2cell('%-SaveRFIQ');

% Specify factor for converting sequenceRate to frameRate.
frameRateFactor = 4;

% Save all the structures to a .mat file.
save([uigetdir,'/VSXinput.mat']);
