    ...
    topDir_Master = ['C:\Users\gwest\Documents\Vantage-4.9.2-2308102000\ElastPhtL74_1607\G218L74_1\']%[uigetdir('C:\Users\gwest\Documents\Vantage-4.9.2-2308102000\PhantomExperimentsL74\','Select Analysis directory'),'\'];

    
    sumIdx = 32;
    
    fileNames = ls(topDir_Master);
    
    %iImage = input('Image # : ');
    
    bfImgData =  load([topDir_Master,'BFimgData',num2str(iImage),'.mat']);
    
    
    lWidth = (bfImgData.Trans.ElementPos(2,1)-bfImgData.Trans.ElementPos(1,1))*bfImgData.lambda;
    ax0 = axes;
    B80 = bmode(bfImgData.iq',80);
    imagesc(ax0,lWidth.*(1:128),bfImgData.yVals,B80);
    ...
    for iFile = 3:size(fileNames,1)
        foldBool = isfolder([topDir_Master,'\',fileNames(iFile,:)]);
    
        if foldBool
            [topDir_Master,'\',fileNames(iFile,:)],
            ls([topDir_Master,'\',fileNames(iFile,:)])
        end
    end
    ...
    hold on 
    plot(ax0,lWidth.*[1 size(bfImgData.fullIM,1)],yVals(depthIdx).*[1 1],'-','color','red')
    ...
    hold on 
    plot(ax1,lWidth.*[1 size(bfImgData.fullIM,1)],yVals(depthIdx).*[1 1],'-','color','red')
    ...
    F = griddedInterpolant(bfImgData.xQcoh,bfImgData.yQcoh,cohValues2);
    CoherenceQ = F(bfImgData.xQbm,bfImgData.yQbm);
    ...
    ylabel('Axial Position (m)')
    hold on 
    plot(lWidth.*[1 size(bfImgData.fullIM,1)],yVals(depthIdx).*[1 1],'-','color','red')
    ...
    subplot(2,2,3) 
    focusEnv = abs(envelope(bfImgData.fullIM(rayIdxs,axIdxs)'));
    ...
    plot(lWidth.*[1 size(bfImgData.fullIM,1)],1.91.*[1 1],'-','Color','red')
    ylabel('SNR')
    xlabel('Lateral Position')
    xlim(lWidth.*[1 128])
